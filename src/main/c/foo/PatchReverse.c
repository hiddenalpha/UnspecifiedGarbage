/*
  ...
  && mkdir -p build/bin \
  && ${CC:?} -c -o /tmp/TnYCAB1QAgChFQIA src/main/c/foo/PatchReverse.c ${CFLAGS:?} -DPROJECT_VERSION=${PROJECT_VERSION:?} \
  && ${LD:?} -o build/bin/PatchReverse$BINEXT /tmp/TnYCAB1QAgChFQIA ${LDFLAGS:?} \

  && bullshit=$(${OBJDUMP?} -p build/bin/PatchReverse$BINEXT|grep DLL\ Name|egrep -v ' (KERNEL32.dll|SHELL32.dll|WS2_32.dll|ADVAPI32.dll|msvcrt.dll)$'||true) \
  && if test -n "$bullshit"; then printf '\n  ERROR: Bullshit has sneaked in:\n\n%s\n\n' "$bullshit"; rm build/bin/PatchReverse$BINEXT; false; fi \
